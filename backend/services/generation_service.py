...
"""
    ...
    def _generate_readme(
        ...
        *,
        ...
        """
        Deterministic, rule-based README generator.
        ...
        project_name = f"{lang_label} {fw_label}".strip() or "InfraScribe Project"
        ...
        lines.append("")
        lines.append(f"> Generated by **InfraScribe** on {generation_date}")
        lines.append("")
        lines.append(
            "This repository contains a **deterministic, production-style DevOps scaffold** "
            f"for **{project_name}**."
        )
        lines.append("")
        lines.append(
            "**⚠️ Important:** This is a Day-0 starting point. You own this code. "
            "Review all variables and configurations before deploying to production."
        )
        lines.append("")
        ...
        # Contract / Philosophy
        # --------------------------------------------------
        lines.append("## 🧾 What InfraScribe Does (and Does Not)")
        lines.append("")
        lines.append("- ✔ Generates standard Docker, Kubernetes, CI/CD, and Terraform files.")
        lines.append("- ✔ Uses predictable, reviewable templates (deterministic output).")
        lines.append("- ✘ Does **not** apply infrastructure changes.")
        lines.append("- ✘ Does **not** access your cloud account.")
        lines.append("- ✘ Does **not** lock you into proprietary formats.")
        lines.append("")
        lines.append("You fully own and control every file in this repository.")
        lines.append("")

        # --------------------------------------------------
        # Stack & Metadata
        # --------------------------------------------------
        lines.append("## Stack and Configuration")
        lines.append("")
        lines.append(f"- **Application stack:** {project_name}")
        ...
        if has_argocd:
            lines.append("├── gitops/")
        if has_terraform:
            lines.append(f"├── infra/terraform/{infra_preset}/")
        if has_monitoring:
        ...
        if has_terraform:
            lines.append("- Terraform CLI installed and cloud credentials configured.")
        if has_k8s or has_helm or has_argocd:
            lines.append("- Kubernetes cluster and `kubectl` configured.")
        if has_helm:
            lines.append("- Helm CLI installed.")
        if has_argocd:
            lines.append("- ArgoCD installed and connected to your Git repository.")
        lines.append("")
        ...
        if has_dockerfile:
            ...
            lines.append("")
            lines.append("```bash")
            lines.append("docker build -t your-image:local .")
            lines.append("docker run -p 5000:5000 your-image:local")
            lines.append("```")
            lines.append("")
        ...
        # --------------------------------------------------
        if has_cicd:
            lines.append("## 2. CI/CD Pipeline")
            lines.append("")
            lines.append(
                "A CI/CD pipeline is included to build and push the Docker image on every push."
            )
            lines.append(
                "You must configure registry credentials (e.g., Docker Hub, ECR) "
                "in your CI system."
            )
            lines.append("")
        ...
        if has_k8s or has_helm:
            ...
            if has_helm:
                lines.append("```bash")
                lines.append("helm upgrade --install app ./helm")
                lines.append("```")
                lines.append("")
        ...
        if has_argocd:
            ...
            lines.append("")
            lines.append(
                "The `gitops/` directory contains ArgoCD Application manifests "
        ...
        # --------------------------------------------------
        if has_terraform:
            lines.append("## 5. Infrastructure with Terraform")
            lines.append("")
            lines.append(
                f"Terraform configuration is provided under `infra/terraform/{infra_preset}/`."
            )
            lines.append("")
            lines.append("```bash")
            lines.append(f"cd infra/terraform/{infra_preset}")
            ...
            lines.append("")
            lines.append(
                "> ⚠️ **Warning:** Review IAM, networking, and security settings "
        ...
        lines.append("")
        lines.append("- Containers run as a **non-root user** by default.")
        lines.append("- No credentials are hard-coded.")
        lines.append("- All configurations are intended to be reviewed and hardened.")
        lines.append("")
        ...
        lines.append("")
        lines.append("- Replace placeholder values (image names, ARNs, domains).")
        lines.append("- Test in a non-production environment first.")
        lines.append("- Gradually harden security, scaling, and observability.")
        lines.append("")
        lines.append("**InfraScribe removes the blank page — you remain the engineer.**")
